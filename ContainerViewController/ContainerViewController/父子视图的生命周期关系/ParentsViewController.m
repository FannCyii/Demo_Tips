...
- (void)removeChildVC
{
    [self.childVC didMoveToParentViewController:nil];
    [self.childVC.view removeFromSuperview];
    //[self.childVC removeFromParentViewController];//在执行removeFromParentViewController方法前会自动调用didMoveToParentViewController方法（注意如果自定义这些方法，必须调用其对应的super方法）。调用 child 的removeFromParentViewController方法后可以查看 parent的childcontainers 发现为空，或者查看child的parentviewcontroller也为空；如果再调用该方法则没有任何效果。
